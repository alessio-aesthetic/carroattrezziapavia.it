...
const vehicles = [
  { name: 'Auto', service: 'traino-auto', title: 'L’attenzione che la tua auto merita.', text: 'Guasto, avaria o auto non marciante: descrivi il problema e concordiamo il recupero e la destinazione.', image: 'selector-auto' },
  { name: 'Moto', service: 'carroattrezzi-moto', title: 'Due ruote. Un trasporto dedicato.', text: 'Moto e scooter richiedono punti di ancoraggio e fissaggi specifici. Comunica modello e condizioni del mezzo.', image: 'selector-moto' },
  { name: 'Veicoli incidentati', service: 'recupero-auto-incidentata', title: 'Prima la sicurezza. Poi il recupero.', text: 'Indica eventuali danni, ruote bloccate e spazio disponibile. Valutiamo come movimentare il veicolo.', image: 'selector-incidente' },
]
export function VehicleSelector() {
  ...
  return <div className="vehicle-selector" data-reveal>
    ...
    <div id="vehicle-panel" role="tabpanel" aria-labelledby={`vehicle-tab-${selected}`} tabIndex={0} className="vehicle-panel">
      <OriginalPhoto key={panel.image} name={panel.image} alt={`Recupero ${panel.name.toLowerCase()}`} sizes="(max-width: 680px) 88vw, 28vw" />
